@extends('layouts.auth')
    ...
    @section('content')
        ...
        <div class="login-header">
            <div class="brand">
                {{-- <span class="logo"></span> --}} <img src='{{ asset('mupo/img/mupo.png') }}' /> {{ __('Login') }}
        ...
        <div class="login-content">
            ...
            <form data-parsley-validate="true" class="margin-bottom-0" method="POST" action="{{ route('login') }}">
                ...
                <div class="register-buttons">
                    <button type="submit" class="btn btn-info btn-block btn-lg"> {{ __('Login') }} </button>
                </div>
                <br />
                <div>
                    <a href="{{ route('password.request') }}" class="btn btn-white btn-block">{{ __('Forgot Your Password?') }}</a>
                </div>
